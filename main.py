...
# --- LIBRERÍAS DE OPENAI (Configuradas para detectar OPENAI_API_KEY) ---
...
app = FastAPI(title="Forex AI Agent - OpenAI Standard")

# --- CONFIGURACIÓN DE RUTAS ---
DOCUMENTS_DIR = "documents"
DATABASE = "forex_memory.db"

# --- ESTRATEGIA DE ACENTOS (Elena - Forex) ---
COUNTRY_ADAPTATION = {
...
def process_message(user_id, message, country_code="default"):
    try:
        ...
        # RAG: Búsqueda en los documentos de inversión
        contexto = ""
        ...
        # LLM: Detecta OPENAI_API_KEY automáticamente de Railway
        llm = ChatOpenAI(
            model="gpt-4o-mini",
            temperature=0.6,
            max_tokens=80
        ...
        TONO: Acento {style['acento']}. {style['jerga']}
        
        TAREA: Persuade al cliente sobre la oportunidad de inversión.
        REGLA DE ORO: Máximo 20 palabras. Responde en el idioma original del usuario.

        CONTEXTO ESTRATÉGICO:
        {contexto if contexto else "Usa argumentos de rentabilidad y seguridad en mercados financieros."}
        """
        ...
        logging.error(f"Error Forex OpenAI: {e}")
        return "¿Qué tal? Me comentabas sobre tu interés en diversificar tu capital, ¿cierto?"

# ---------------- INDEXACIÓN CON OPENAI EMBEDDINGS ----------------
def build_index():
    global vector_db
    try:
        # Detecta OPENAI_API_KEY automáticamente
        ...
        if all_docs:
            ...
            with index_lock:
                ...
                logging.info("--- DOCUMENTOS DE FOREX INDEXADOS CON OPENAI ---")
    except Exception as e:
        logging.error(f"Error en indexación Forex: {e}")
...
async def vapi_webhook(request: Request):
    try:
        ...
        logging.error(f"Error Webhook Forex: {e}")
        return {"error": str(e)}
...
@app.get("/")
def health_check():
    return {"status": "Elena Forex Online", "config": "Standard OpenAI Key"}
